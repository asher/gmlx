...
derives code from the following projects. The files taken from them keep
their project's license. Full license texts, with per-file provenance, live
...
| [mlx-vlm](https://github.com/Blaizzy/mlx-vlm) | MIT | The chunked-eval `store_kv_blocks` override in `gmlx/cache/apc_manager.py`, the thinking-budget logits processor in `gmlx/gen/thinking_budget.py`, and `entropy_transfer_mask` in `gmlx/systemone/denoise.py`. The installed package is also a runtime dependency. |
...
| [vLLM](https://github.com/vllm-project/vllm) | Apache-2.0 | The structured-decision logic in `gmlx/systemone/` (schema, answer templates, decisions, answer shapes) is ported from `examples/features/structured_diffusion/structured_server.py`, and the unembedding, slot log-probabilities and multi-step read loop in `gmlx/systemone/denoise.py` from `vllm/model_executor/models/diffusion_gemma.py`, both at commit `ab3de6edf2`. The parity test vendors `structured_server.py` as `tests/systemone/_systemone_proxy_ref.py`. |
