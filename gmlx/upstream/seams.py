...
SEAMS: tuple[Seam, ...] = (
    ...
    Seam("mlx_vlm.models.switch_layers", "SwitchGLU.__call__",
         ...
         critical=True, module_optional=True),
    # --- structured reads on DiffusionGemma (systemone) ---
    # The read presents a batch-N prompt K/V to the stock decoder through
    # per-layer views that carry keys, offset and decoder_state.
    Seam("mlx_vlm.models.diffusion_gemma.language", "Attention.__call__",
         "systemone.engine (decoder reads _cache_state, SDPA without cache)"),
    Seam("mlx_vlm.models.diffusion_gemma.language", "DecoderModel.__call__",
         "systemone.engine (canvas forward with cache views)"),
    Seam("mlx_vlm.models.diffusion_gemma.language",
         "DecoderModel._make_decoder_masks",
         "systemone.engine (views make both masks None)"),
    Seam("mlx_vlm.models.diffusion_gemma.language", "DecoderModel._embed_canvas",
         "systemone.engine (self-conditioning input contract)"),
    Seam("mlx_vlm.models.diffusion_gemma.language", "SelfConditioning.__call__",
         "systemone.engine (soft embedding shape)"),
    Seam("mlx_vlm.models.diffusion_gemma.language", "_cache_state",
         "systemone.engine (view K/V source, decoder_state attribute)"),
    Seam("mlx_vlm.models.diffusion_gemma.language", "_cache_offset",
         "systemone.engine (view offset attribute)"),
    Seam("mlx_vlm.models.diffusion_gemma.language", "EncoderModel.make_cache",
         "systemone.engine (prompt cache layer types)"),
    Seam("mlx_vlm.models.diffusion_gemma.language",
         "DiffusionGemma4Backbone.diffusion_prefill_cache",
         "systemone.engine.StructuredReader.prefill"),
    Seam("mlx_vlm.models.diffusion_gemma.language",
         "DiffusionGemma4Backbone.diffusion_update_cache",
         "systemone.engine.PromptCache.extend"),
    Seam("mlx_vlm.models.diffusion_gemma.language",
         "DiffusionGemma4Backbone.diffusion_decoder_masks",
         "systemone.engine (mask contract the views rely on)"),
    Seam("mlx_vlm.models.diffusion_gemma.language", "make_compiled_softcap",
         "systemone.engine (model._softcap on the read logits)"),
    Seam("mlx_vlm.models.diffusion_gemma.diffusion_gemma", "Model.__init__",
         "systemone.engine (owner of _softcap)"),
    Seam("mlx_vlm.models.cache", "RotatingKVCache._update_concat",
         "systemone.engine.PromptCache.extend (multi-token append)"),
    Seam("mlx_vlm.models.cache", "RotatingKVCache._update_in_place",
         "systemone.engine.PromptCache.extend (one-token append)"),
    Seam("mlx_vlm.models.cache", "RotatingKVCache._temporal_order",
         "systemone.engine (sliding trim assumes oldest-first order)"),
    Seam("mlx_vlm.generate.common", "generation_stream",
         "systemone.engine.engine_scope"),
    Seam("mlx_vlm.generate.diffusion", "stream_diffusion_generate",
         "systemone.engine.StructuredReader.think"),
    Seam("mlx_vlm.generate.diffusion", "_diffusion_entropy_transfer_mask",
         "systemone.engine.entropy_transfer_mask (owned copy; re-mirror "
         "on upstream change)"),
    Seam("mlx_lm.generate", "wired_limit",
         "systemone.engine.engine_scope (the gmlx-patched policy)"),
    Seam("mlx_vlm.server.generation", "ResponseGenerator._generate_diffusion",
         "engine_jobs.install_engine_jobs (wrapped: a queued job runs in "
    ...
    Seam("mlx_vlm.server.generation", "GenerationContext",
         "engine_jobs.run_on_engine (first queue item)"),
    Seam("mlx_vlm.server.generation", "get_token_queue_timeout",
         "patches.systemone (decision deadline)"),
    Seam("mlx_vlm.server.generation", "_build_metrics_envelope",
         "patches.systemone (request log line)"),
    # --- mlx-lm seams ---
