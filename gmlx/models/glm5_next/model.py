...
def _dequantized(kv, cache):
    ...
    if isinstance(kv, tuple):
        # Cache fetches are lazy step-buffer slices. mlx < 0.32.2 encoded
        # a strided biases operand's contiguity copy after binding w and
        # scales, so dequantize read the copy kernel's buffers (mlx
        # #4381). Permanent wraps: free when already contiguous.
