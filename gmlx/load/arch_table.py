...
_FAMILY_NOTES = {
    ...
    "nemotron_h_moe": ("nemotron", "NVIDIA Nemotron-H MoE hybrid (Nemotron-3-Super, Nemotron-3.5-Lightning): Mamba2 SSM layers + NoPE attention layers + MoE MLPs; layer_norm_epsilon (not rms_norm_eps). Lightning appends an in-file NextN/MTP block (blk.{block_count-1}: combined attention+MoE + nextn.* fusion, excluded from the trunk) wired for --speculative; llama.cpp also ships it as a same-arch mtp-*.gguf sidecar usable via --draft-gguf"),
    ...
    "deepseek41":     ("deepseek", "DeepSeek-V4.1-Flash (384x8.4B, llama.cpp PR #28696 convert patch, or the ds4 converter). The V4 skeleton with every block changed: engram conditional memory on layers 1 and 14 (a 384M-row x 256 hash-keyed n-gram table each, 24 rows per token from the last 4 token ids, gated into the 4 residual streams by a normalized stream-key dot product), hyper-connections whose collapse lags one sublayer (attention collapses with the previous FFN's pre-mix; no final-collapse head), and CSA2 attention: a 128 sliding window on every layer plus compressed-KV and lightning-index streams that 4 kv-source layers own and the layers after them read, pooling ratios 0/1/2, no post-projection q norm, no compressor positional table. Every-layer 384-expert MoE, sqrt-softplus gating, selection-only correction bias (with an image-token variant), 1 shared expert, clamped SwiGLU, no hash routing. Engram tables and routed experts both stream from SSD; the tables go through the table-streaming tier, the experts through the arena. DSML tool calls with leading-space tag names and a numeric 1-100 reasoning-effort budget; gmlx ships the V4.1 chat template because the GGUF embeds the V4 one. The vision encoder GGUF (--mmproj, arch deepseek4-vision) carries raw HF tensor names with fused qkv and gate/up rows rather than clip.* keys: the same DeepSeek ViT (2-D rope) + 3x3 unfold aligner as V4, with an image budget of 1024 tokens and no width cap. Each image becomes a block of sentinel ids past the vocab (START, grid rows each closed by a NEWLINE, END) in plain reading order with no lead pads and no pooling, so the text side attends causally and a block may be split across prefill chunks; the image router bias and the engram gate mask cover every block position. Model class vendored (no upstream mlx-lm class)"),
    ...
    "hyv4":           ("hunyuan",  "Tencent HY4-preview (256x29B, llama.cpp LLM_ARCH_HYV4); absorbed MLA in the DeepSeek-V3.2 shape (q_lora 2048, kv_lora 512, MLA head 256 = nope 192 + rope 64, head_count_kv 1, scale 256**-0.5) with three additions: per-head learnable attention sinks folded into the softmax normalizer, a sigmoid gate on the decompressed attention output before o_proj, and a per-token DSA lightning indexer (32 heads x 128 dims, top-k 2048, rope on the last 64 dims - the reverse of deepseek_v32) on the 21 layers marked full by attention.indexer.is_full; the other 57 layers reuse the most recent preceding full layer's selection and carry no indexer weights. Below 2048 cached keys the selection is the identity and attention is dense. iHC (independent hyper-connections): 4 parallel residual streams, one [4*hidden -> 8] mixer per sublayer producing 4 pre gates that collapse the streams and 4 post gates (magnitude 2.0) that redistribute the sublayer output, all fp32, with no sinkhorn/comb term - the delta from deepseek4 - and a learned collapse head before the final norm. Sigmoid MoE (256 experts top-8, selection-only correction bias, renorm x 2.827, 1 shared expert, 1 leading dense layer) with clamped SwiGLU (limit 10) on the routed experts only - the shared and dense MLPs are plain. Interleaved rope (LLAMA_ROPE_TYPE_NORM, matching vLLM is_neox_style=False; transformers 5.15 switched to rotate_half and llama.cpp deliberately did not follow) at theta 1e7, no scaling. gpt2 byte-level BPE on the deepseek3-llm 3-regex pretokenizer; every control tag carries a :6124c78e suffix. No MTP: the converter drops the nextn layers, so this family cannot drive speculative decoding. --kv-bits is refused (quantized SDPA has no sinks argument). Model class vendored (no upstream mlx-lm class)"),
...
MTP_WIRED_MODEL_TYPES = frozenset({
    ...
    "muse_glimmer", "qwen4_exp", "glm5_next", "nemotron_h",
})
...
_VENDORED_MLX_LM_MODULES = {
    ...
    # llama.cpp LLM_ARCH_HYV4 (HY4-preview); no upstream mlx-lm class
    # (deepseek_v32 is the nearest relative and carries neither the
    # independent hyper-connections nor the sinks and the attention gate).
    "hy_v4": "gmlx.models.hy_v4.model",
}
