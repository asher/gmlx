...
_FAMILY_NOTES = {
    ...
    "deepseek41":     ("deepseek", "DeepSeek-V4.1-Flash (384x8.4B, llama.cpp PR #28696 convert patch). The V4 skeleton with every block changed: engram conditional memory on layers 1 and 14 (a 384M-row x 256 hash-keyed n-gram table each, 24 rows per token from the last 4 token ids, gated into the 4 residual streams by a normalized stream-key dot product), hyper-connections whose collapse lags one sublayer (attention collapses with the previous FFN's pre-mix; no final-collapse head), and CSA2 attention: a 128 sliding window on every layer plus compressed-KV and lightning-index streams that 4 kv-source layers own and the layers after them read, pooling ratios 0/1/2, no post-projection q norm, no compressor positional table. Every-layer 384-expert MoE, sqrt-softplus gating, selection-only correction bias (with an image-token variant), 1 shared expert, clamped SwiGLU, no hash routing. Engram tables and routed experts both stream from SSD; the tables go through the table-streaming tier, the experts through the arena. DSML tool calls with leading-space tag names and a numeric 1-100 reasoning-effort budget; gmlx ships the V4.1 chat template because the GGUF embeds the V4 one. The vision encoder GGUF (--mmproj, arch deepseek4-vision) carries raw HF tensor names with fused qkv and gate/up rows rather than clip.* keys: the same DeepSeek ViT (2-D rope) + 3x3 unfold aligner as V4, with an image budget of 1024 tokens and no width cap. Each image becomes a block of sentinel ids past the vocab (START, grid rows each closed by a NEWLINE, END) in plain reading order with no lead pads and no pooling, so the text side attends causally and a block may be split across prefill chunks; the image router bias and the engram gate mask cover every block position. Model class vendored (no upstream mlx-lm class)"),
...
_CAVEATS = {
    ...
    "deepseek41": "No MTP drafter: the GGUF carries no DSpark tensors.",
