...
def _add_serve_args(ap: argparse.ArgumentParser) -> None:
    ...
    ap.add_argument("--dtype", choices=("auto", "bfloat16", "float16"),
                    ...
                    help="Activation dtype for every model this server loads "
                         "(default on this test branch: float16; shipping "
                         "gmlx defaults to bfloat16). float16 is for M1 and "
                         "M2, whose GPUs have no native bfloat16 arithmetic; "
                         "auto picks it on those and bfloat16 elsewhere. Also "
                         "via GMLX_ACTIVATION_DTYPE; config mode: server.dtype.")
