...
absolute imports, and two deliberate divergences:

- ``sanitize`` STRIPS the MTP block(s) from the trunk (PR #1211 semantics)
  instead of loading them onto an inline ``Model.mtp`` submodule, and the
  PR's ``MTPBlock`` class is dropped with it. gmlx builds MTP drafters as
  separate objects bound to the target - the Hy3 head lives in
  ``hy_v3_mtp.HyV3MTPDrafter`` (see ``loader.remap_mtp_arrays`` and
  ``drafter_protocol``) - so the trunk model never carries draft weights.
- The MTP head consumes the POST-final-norm hidden state. PR #1485 feeds the
  pre-norm residual into ``hnorm``; llama.cpp PR #25395 verified the wiring
  line-by-line against vLLM's reference (hy_v3_mtp.py + the v1 runner) and
  cross-checked behaviorally (46.9% vs vLLM's 46.7% ungated acceptance), and
  that reference normalizes first: h_nextn = final_norm(h). ``Model.__call__``
  with ``return_hidden_states=True`` therefore returns the post-norm hidden.

Hy3 (299B-A21B): 80 layers, GQA 64/8 heads at head_dim 128 with per-head
qk-norm, plain rope (theta 11.16M); layer 0 dense SwiGLU, layers 1-79
fine-grained MoE - 192 experts top-8, sigmoid gating + selection-only expert
bias, top-k renorm x router_scaling_factor, one ungated shared expert. One MTP
...
class Model(nn.Module):
    ...
    def __call__(
        ...
        if return_hidden_states:
            # Post-final-norm: what the MTP block's hnorm consumes (see header).
            return self._logits(out), out
    ...
    def sanitize(self, weights):
        ...
        # The trunk never carries the MTP block(s); the speculative drafter
        # loads them separately (loader.remap_mtp_arrays).
        if n_mtp > 0:
