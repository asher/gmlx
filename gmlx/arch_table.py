...
_FAMILY_NOTES = {
    ...
    "deepseek2":       ("deepseek", "DeepSeek-V3/R1, Moonshot Kimi-K2.5/K2.7, + GLM-4.x MLA conversions (GLM-4.7-Flash); MLA attention (absorbed embed_q/unembed_out via KQuantMultiLinear, native Q8_0), fine-grained sigmoid-gated MoE + shared expert + group routing, leading dense block. The converter writes rope yarn_log_multiplier as 0.1 * mscale_all_dim, which the synth undoes (reading it raw flattens attention ~1.85x). Kimi K2.x is ungrouped (384 experts, one group) and refines onto its own sampling family (t=1.0) off general.name; its <|im_user|>/<|im_middle|> chat format leaves <think> open at the generation prompt and the stock kimi_k2 tool parser reads its tool calls. Pairs with the Kimi K2.x MoonViT mmproj (--mmproj, projector 'kimik25') for vision. V2 (softmax gating) not yet supported"),
