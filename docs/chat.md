...
| `/reset`, `/clear` | restart the conversation, and `/clear` also wipes the screen |
...
| `/! <command>` | run a shell command and stage its output for the next message, and `/drop` discards staged blocks |
...
Arrow keys, Ctrl-A and Ctrl-E edit the line. Up-arrow history persists
across sessions under `$XDG_CACHE_HOME/gmlx/`, and `--no-history` keeps a
...
blocks, context fill and the last reply's speed. Multi-line paste is
...
interrupts the command, not the session.
...
companion drafter. The reply streams as before, and the KV cache is reused
across turns exactly as on the plain path. On that path sampling is limited
to temperature, top-p, top-k and min-p, since the verify step has no
penalty or bias hooks.
...
| style keys in a slot | booleans `bold`, `dim`, `italic` and `underline`, `fg16` as an ANSI code 30-37 or 90-97, and `rgb` as truecolor with a 256-color fallback that overrides `fg16` |
