...
Generated by `scripts/check-coverage.py` from the architecture table and the
installed mlx-lm. Do not edit by hand.

40 of 41 mapped architectures load end-to-end with no `hf_source`.
...
The caveat column names the limit a user is most likely to hit. An empty cell
means none is known.
...
| `deepseek41` | deepseek | mlx-lm | loadable | Text only: no mmproj is published yet. No MTP drafter: the GGUF carries no DSpark tensors. |
