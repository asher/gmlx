...
| `GMLX_GPU_RESIDENT=0` | Skip wiring the non-expert spine into the Metal residency set on streamed installs (default on: command buffers otherwise re-wire the spine's pages on every use). |
