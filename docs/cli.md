...
| `GMLX_QWEN_OWNED=0` | Build qwen3.5/3.6 MTP targets on the stock mlx-vlm class instead of the owned model-level forward. Read at load; token identical either way, so `0` is a debugging A/B, not a tuning knob. |
