...
| `--stream-cpu` | Run the whole model on the CPU device: the over-RAM MoE path kept entirely on one device. Weights stay mmap-backed so the page cache streams them from disk; past the GPU wired budget the runtime adds sequential expert prefetch (`GMLX_STREAM_PREFETCH=0` disables) and a wider default prefill chunk (8192). When and why: [performance.md](performance.md#bigger-than-memory-moe-offload). |
| `--stream-experts` | Routed-expert stacks stream from disk while the every-token layers (attention, norms, routers, shared experts) and the KV cache stay on GPU. With the decode feeder (default) it matches `--stream-cpu` on short generations and pulls ahead as the arena warms; a quantized KV cache (`--kv-bits`) extends the win to long context. Mutually exclusive with `--stream-cpu`. Details: [performance.md](performance.md#bigger-than-memory-moe-offload). |
...
| `--moe-expert-mass P` | Lossy, adaptive: per token, keep only the smallest set of routed experts covering share P (0 < P <= 1) of the router's gate mass - confident tokens read fewer expert bytes, uncertain tokens keep the full fan-out. Choosing P: [performance.md](performance.md#bigger-than-memory-moe-offload). |
...
| `--[no-]prefill-feeder` | Streaming models (`--stream-cpu` / `--stream-experts` past the wired budget): stage each prefill layer's expert stacks straight from the GGUF into GPU-visible ring slots instead of through the page cache - one trip per byte, and short prompts stage only the experts the router chose. Default on; `--no-prefill-feeder` falls back to page-cache prefetch. Details: [performance.md](performance.md#bigger-than-memory-moe-offload). |
| `--[no-]decode-feeder` | Streaming `--stream-experts` models: serve decode from a wired, popularity-managed GPU expert arena; misses read from the GGUF at SSD queue depth. Default on for `--stream-experts` (needs the every-token layers on GPU, so never under `--stream-cpu`); `GMLX_DECODE_ARENA_GB` caps the arena size. Details: [performance.md](performance.md#bigger-than-memory-moe-offload). |
...
| `GMLX_NATIVE_FP` | Layout for MXFP4/NVFP4 expert tensors: `wire` (zero-copy GGUF wire bytes, loads in seconds), `packed` (eager repack into MLX's layout), or the default `auto` (wire when a streaming placement is requested or the file nears the wired budget). See [performance.md](performance.md#bigger-than-memory-moe-offload). |
