...
| `GMLX_TRAIN_GDN_CHUNK=0` | Run the gated delta scan of a training forward (owned Qwen3.5, 3.6 and Qwen4 experimental) on mlx-lm's per-token loop instead of the chunked rule in `gmlx.tune.gdn`. Both sit inside a per-layer checkpoint. Same numerics to float32 rounding; the loop costs one sequential step per token. |
