...
| `GMLX_TRAIN_BLOCKED_ATTN=0` | Run training attention on MLX's unfused path instead of the query-block recompute in `gmlx.tune.attention`, which is the default for the owned Qwen3.5 and 3.6 forward and for every stock attention `gmlx train` patches. Same numerics to rounding; the unfused path keeps every layer's full softmax on the tape. |
| `GMLX_TRAIN_GDN_CHUNK=0` | Run the gated delta scan of a training forward (Qwen3.5, 3.6, Qwen3-Next and Qwen4 experimental) on mlx-lm's per-token loop instead of the chunked rule in `gmlx.tune.gdn`. Both sit inside a per-layer checkpoint. Same numerics to float32 rounding; the loop costs one sequential step per token. Per-key-channel decay (Kimi-K3, GLM-5-Next) always takes the loop. |
| `MLX_ENABLE_TF32=1` | Keep MLX's TF32 float32 matmul in `gmlx train` and `gmlx distill`, which otherwise set it to `0` before the first matmul. The chunked gated delta rule diverges under TF32 rounding, so with TF32 on it takes the loop and prints why. |
