...
with no conversion. This page maps the rest: what transfers directly, what
has a different name, and what is different by design.
...
| `--rope-scaling`, `--yarn-*` | none | metadata-driven; an expert override exists in [debug-switches.md](internals/debug-switches.md) |
...
- gmlx implements the OpenAI, Anthropic and OpenAI Responses APIs, not the Ollama
...
- Modelfile parameters map onto the config: `num_predict` is the server's
  `max_tokens` default, sampling keys are set per model or in `profiles:`, and
...
exists. Ids, sampling profiles and a default model can then be adjusted
in one YAML file. The local server API is the same as LM Studio's,
