...
ones, and clicking a model unloads it. The menu offers reload config,
restart, stop, copy URL and open logs, all through the server's endpoints.
If a tracked server exits unexpectedly the item posts a macOS notification,
while an intentional stop or restart does not.
...
login item with [gmlx service install](cli.md#gmlx-service), which also
makes macOS permission prompts attribute to gmlx instead of your terminal.

Like `serve`, the item detaches by default, although `--foreground` runs
the event loop in place and `--stop` quits a detached item. Only one item
runs on a machine, so a second `serve` or a manual launch is a no-op. With
no explicit target it tracks the primary server, following it as servers
start and stop, while `--url`, `--host` or `--port` restricts it to one
server. It reads the API key from the managed server's config, or takes
`--api-key` for a server whose config it cannot see, and a key-protected
server it has no key for shows as up with a key-required note. The flags are
listed under [launch menubar](cli.md#launch-menubar).
...
editing, and Save and Reload validates, saves and triggers the running
...
advance and exempt from the idle timeout, so at no point is the mic open
while the model is unloaded. When the session ends the hold is released,
although the model is not evicted.
...
is detected within a few seconds, and if arming still fails right after a
grant, the app says to quit and reopen the menu bar, since some macOS
versions bind grants only to a freshly launched process. Permission prompts
attribute to gmlx when the bar runs as the login item, whereas a bar
launched from a terminal runs under the terminal's identity and grants then
apply to the terminal app.
