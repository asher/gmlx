...
the background afterward. It is built for errand-sized tasks: look something
up, chain a few tool calls, write a note, remember a fact. The coding agents
`gmlx launch` connects bring their own loops and use this server for
inference only.
...
final tool-less request forces an answer, so a tool-happy model cannot loop
...
a good fit on a 48 GB or larger machine; Qwen3.5-9B is a workable floor on
...
to come up degrades to a warning rather than blocking the loop, and a missing
...
from your shell is inherited, so a token sitting in your environment never
reaches third-party tool code unless you pass it.

## Tool recipes

Any stdio or streamable-HTTP MCP server plugs in the same way. These need no
further setup.
...
every turn. A retrieval tool is deliberate and document-shaped: the model
...
as transient context that never bloats the chat history. Without
...
leaves no residue, and a new fact that restates an existing one replaces it.
...
`/memory`. Sampling flags forward to the server per round once you touch
...
and a stream-path cancel lands at the next delta or tool boundary.

## Security

`gmlx talk` and `gmlx chat --assistant` execute tools on the machine you are
sitting at, as you, which is the trust domain of any local CLI tool. A served
...
beyond loopback it means anyone holding the API key can drive tool execution
on the host. The gates that follow:
...
- A non-loopback bind with `server.assistants` configured refuses to start
  unless `server.assistant_allow_remote: true`, on top of the rule that a
...
id, fine for a personal server and wrong for anything multi-user, which is
why it defaults off.
