...
| `GMLX_DECODE_ARENA_GB` | Decode arena size override in GB. Default: what the ceiling leaves after the every-token weights, KV room and prefill ring. |
| `GMLX_DECODE_ARENA_RAM_FRAC` | Cap the arena ceiling at a fraction of physical RAM. No default. |
...
| `GMLX_STREAM_KV_WIDTH` | Concurrent streams the KV room is sized for (default `1`). Each costs arena slots. |
| `GMLX_DECODE_KV_RESERVE_GB` | Replace the priced KV room with a flat reserve in GB; also the fallback of `8` when the header cannot be priced. |
...
| `GMLX_DECODE_PRESSURE=0` | Keep the arena at its sized capacity under memory pressure. Default on: it shrinks, keeping its most popular experts, and regrows when pressure clears. |
| `GMLX_DECODE_RAM_FLOOR_GB` | Host floor kept free for the rest of the box when the arena is sized. Default 5% of RAM, at least `4`. |
| `GMLX_DECODE_PAGECACHE_GB` | Page-cache reserve added to the host floor (default `2.5`). Buffered read paths collapse when the cache is starved. |
...
| `GMLX_KEEPWARM_IDLE_S` | Seconds without streamed decode before the keep-warm heartbeat parks (default `1`; `0` beats continuously). |
...
| `GMLX_GOV_RESERVE_GB` | RAM left to the kernel and other processes when the server's memory ceiling is computed. Default the larger of `8` and 10% of RAM. |
| `GMLX_CACHE_LIMIT_GB` | MLX buffer-cache limit for `serve` in GiB; wins over `server.cache_limit_gb`. Negative, `off`, `none` or `unlimited` unbounds it; `0` disables caching. |
...
| `GMLX_CASCADE_MIN_P` | Smallest shared-prefix length in tokens the cascade route claims (default `1024`). |
