...
needs two services beside the chat model: an embedder to index documents and
queries as vectors, and optionally a reranker to re-score the vector search's
...
yes or no to "does this document satisfy this query", scored as the
probability of yes. Aliases are `qwen3-rerank-0.6b`, `-4b` and `-8b`, or any
GGUF ref; see [Reranking](services.md#reranking-rerank).

## The embeddings endpoint

OpenAI shape. `input` is a string or list of strings, and vectors come back
L2-normalized:
...
field can be omitted. Any other requested model is refused rather than
silently substituted. The response echoes the name you requested. An optional
`encoding_format` selects `float` (default) or `base64`.

## The rerank endpoint

Cohere and Jina shape on `POST /v1/rerank`, also served at `/rerank`. Send
the query and the candidate documents and get back indices sorted best-first
...
`http://127.0.0.1:8080/v1` and any API key unless the server sets one. The
built-in assistant's long-term memory embeds remembered facts through this
