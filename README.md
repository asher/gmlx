...
gmlx needs an Apple Silicon Mac and Python 3.11 or newer, which uv installs
for you if it is missing. Intel Macs and Linux are not supported. On macOS
26.2 or newer the Metal kernels install as a prebuilt wheel. Older versions
build them from source and need full Xcode with its Metal toolchain, which
...
walks through. A model needs memory for roughly its file size plus the
...
Any local `.gguf` runs, chats or serves this way with no other setup. When
you serve a single file, its id is the filename without the quant tag, which
is why the curl above asks for `qwen3-0.6b`. The `--to .` puts the download in
the current directory; once a config exists, `pull` writes to your model
directory and registers the file there instead.

`gmlx[all]` turns on every optional feature. The core install already
serves, loads vision models, embeds and runs the menu bar, so `gmlx[chat]`
leaves out only voice and the assistant. Upgrade with `uv tool upgrade gmlx`.
To remove gmlx, follow
...
cache, lets you rename ids and pick a default, and offers the on-disk prompt
cache and the speech, embedding and rerank services before it writes
...
without asking anything. The full walkthrough is the
...
changes and image input. Both start from the sampling defaults on each model
family's card, and an `@profile` suffix such as `model.gguf@creative`
switches them for a single call. Every flag is listed under its verb in
...
running. The supported clients, listed with their quirks in
[launch.md](https://github.com/asher/gmlx/blob/main/docs/launch.md), are pi,
opencode, omp, claude-code, hermes, goose, aichat, elia and Open WebUI. A menu
