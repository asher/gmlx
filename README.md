...
gmlx is a local inference platform. Chat with an open model in the terminal or
your browser. Serve it over OpenAI and Anthropic compatible APIs. Connect your
coding agent to it, talk to it by voice, build a local RAG stack on it, and
fine-tune it with LoRA.

It runs the community's K-quant and IQ-quant GGUF builds exactly as published.
At equal file size those are the most accurate open quant formats. The
...
benchmarks faster than llama.cpp. The gap is widest at the long contexts that
coding agents and long sessions use. A mixture-of-experts model bigger than
RAM still runs by streaming its experts from disk. If you are moving from
llama.cpp, Ollama or LM Studio,
[migrating.md](https://github.com/asher/gmlx/blob/main/docs/migrating.md) says
what transfers.
...
describes. Python 3.11 or newer is required. uv fetches one for you. A model
needs memory for roughly its file size plus the conversation's KV cache.
...
served id is the filename without its quant tag, `qwen3-0.6b` in this example.
Ids assigned by `gmlx init` keep the tag. `--to .` downloads into the current
directory. Once a config exists, `pull` writes to your model directory and
registers the file there. `gmlx[all]` turns on every optional feature. The
core install already serves, loads vision models, embeds and runs the menu
bar, so `gmlx[chat]` omits only voice and the assistant. Upgrade with `uv tool
upgrade gmlx`. Removing gmlx is described in
...
other commands read. Run with no arguments, it opens a wizard. The wizard
scans the folders you name, including an LM Studio library or a Hugging Face
cache. It lets you rename ids and set a default, offers the on-disk prompt
cache and the speech, embedding and rerank services, then writes
...
choice also has a flag. `gmlx init --models-dir ~/models` writes the config
without asking questions. The [getting-started
...
conversation, `chat` is a multi-turn terminal client with markdown rendering,
sessions, live sampling changes and image input. Sampling defaults come from
each model family's card. `@intents` such as `model.gguf@creative` switch them
for a single call. The flags are listed in
...
fit. It also lists the quants in a repo. Then `pull` downloads sharded files,
...
is paced so live replies keep streaming, while a prompt cache skips repeated
...
without modifying your dotfiles. If the server is not running, it starts it.
...
`train` fine-tunes through the quantized matmul, which means a model too large
for memory in fp16 still trains. It writes the adapter as a GGUF that
llama.cpp reads too. `--adapter` applies it at run, chat or serve. A base can
serve several adapters at once. The guide is
[lora.md](https://github.com/asher/gmlx/blob/main/docs/lora.md).

## Performance

gmlx and llama.cpp run the same file, which makes the comparison direct. On an
M5 Max, gmlx prefills faster on every model in the fleet at every depth. With
...
runs MoE models larger than memory and makes a 200B-class model usable on a 64
GB machine. The choice of file also affects speed. A uniform K-quant decodes
faster than a heavily mixed one at similar quality. K-quants also have less
...
A family is listed in the generated [coverage
table](https://github.com/asher/gmlx/blob/main/docs/arch-coverage.md) only
after token-parity certification against llama.cpp at 16k context. The table
names the one caveat for each architecture. All 19 K-quant, legacy and IQ
codecs load, plus the MXFP4 and NVFP4 pair. Vision models load as a GGUF
...
for the GGUF format and the K-quant reference implementations, and on
[MLX and mlx-lm](https://github.com/ml-explore/mlx-lm) for the runtime and
model implementations. [mlx-vlm](https://github.com/Blaizzy/mlx-vlm) supplies
the batching server engine and the vision towers,
[mlx-whisper](https://pypi.org/project/mlx-whisper/) speech-to-text and
...
your own purposes, including commercial work, but not redistribute or
