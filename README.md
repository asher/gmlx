...
or your browser, serve it over OpenAI and Anthropic compatible APIs, point
your coding agent at it, talk to it by voice, build a local RAG stack on it,
...
published. Size for size those are the most accurate open quant formats, and
...
where coding agents and long sessions live. A mixture-of-experts model
...
feature; the core install already serves, sees images, embeds and runs the
menu bar, so `gmlx[chat]` gives up only voice and the assistant.

## Set up with gmlx init

`gmlx init` finds your GGUF files, names them, and writes the one config
every other command reads. Run bare it opens a wizard: it scans the folders
...
gmlx launch pi            # point a coding agent at the server
```

Every command now takes a model id in place of a path, and every wizard
choice has a flag, so `gmlx init --models-dir ~/models` scaffolds with no
questions. The
[getting-started guide](https://github.com/asher/gmlx/blob/main/docs/getting-started.md)
is the full walkthrough, with model picks per machine size.
...
Serve an API. One port speaks OpenAI Chat Completions, OpenAI Responses and
...
skips repeated prefixes. The server binds loopback by default, takes a static
...
writes the tool's native config without touching your dotfiles and starts
the server if it is down. Supported: pi, opencode, omp, claude-code, hermes,
...
long-term memory, and drives `chat --assistant` and served assistant ids too.
...
model you could not hold in fp16 still trains, and writes the adapter as a
GGUF that llama.cpp reads too. `--adapter` applies it at run, chat or serve,
and one base can serve several adapters side by side.
...
with speculative decoding on both engines decodes ahead throughout. Absolute
numbers scale with the machine's memory bandwidth; measure your own with
`gmlx run model.gguf --bench 128,512,2048`.

The levers, in [performance.md](https://github.com/asher/gmlx/blob/main/docs/performance.md):
...
The file you pick matters too: a uniform K-quant decodes faster than a
...
the table names the one caveat each architecture carries. All 19 K-quant,
...
the tokenizer. The full surface, including preflight and the mlx-lm server
...
Pull requests are welcome. Dev setup and the ground rules are in
...
model zoo, [mlx-vlm](https://github.com/Blaizzy/mlx-vlm) for the batching
...
after its release. Downloaded model weights carry their own licenses.

The files listed in
[LICENSE-MIT](https://github.com/asher/gmlx/blob/main/LICENSE-MIT) are MIT
licensed and carry an SPDX header saying so. Vendored third-party code is
documented in
